pipeline {
	...
	stages {
		...
		stage ('Push image'){
			steps{
				echo 'Pushing image ....'
                                sh 'cd /var/lib/jenkins/workspace/docker-loadbalancer/ && echo "$DPASS" | docker login --username uliorea --password-stdin  && docker tag loadbalancer:latest uliorea/myimages:loadbalancerngixn && docker push uliorea/myimages:loadbalancerngixn'
			}
			post {
				success {
